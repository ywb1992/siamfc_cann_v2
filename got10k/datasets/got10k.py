...
class GOT10k(object):
    ...
    def __init__(self, root_dir, subset='test', return_meta=False):
        ...
        list_file = os.path.join(root_dir, subset, 'list.txt') # 获得 list.txt，其中包含了所有视频文件夹名
        with open(list_file, 'r') as f:
            self.seq_names = f.read().strip().split('\n')  #按行读取所有的视频文件夹名
        self.seq_dirs = [os.path.join(root_dir, subset, s) #获取所有视频文件的绝对路径
                         for s in self.seq_names]
        self.anno_files = [os.path.join(d, 'groundtruth.txt') # 获取所有视频文件的标注框文件的绝对路径
    ...
    def __getitem__(self, index):
        ...
        # 获得第 index 个视频序列下所有 *.jpg 文件的绝对路径
        img_files = sorted(glob.glob(os.path.join(
            self.seq_dirs[index], '*.jpg')))
        # 获得第 index 个视频序列下所有 groundtruth.txt 文件内的标注 [x, y, w, h]
        ...
        if self.subset == 'test' and anno.ndim == 1:
            ...
            assert len(img_files) == len(anno) # 验证长度是否一致

        if self.return_meta:
            meta = self._fetch_meta(self.seq_dirs[index]) # 获取 meta 信息

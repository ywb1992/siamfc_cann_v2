...
class ExperimentOTB(object):
    ...
    def __init__(self, root_dir, version=2015,
                 result_dir='results', report_dir='reports', download=False):
        super(ExperimentOTB, self).__init__()
        self.dataset = OTB(root_dir, version, download=download) #默认不下载
        self.result_dir = os.path.join(result_dir, 'OTB' + str(version))
        self.videos_dir = os.path.join(result_dir, 'OTB' + str(version), 'videos')
    ...
    def _video_save(self, imgs, seq_names):
        result_video_path = os.path.join(self.videos_dir, seq_names + '.mp4')
    ...
    def _record(self, record_file, boxes, times, ce_per_frame):
        ...
        # record center errors
        if ce_per_frame is not None:
            ce_dir = os.path.join(record_dir, 'center_errors')
            if not os.path.isdir(ce_dir):
                os.makedirs(ce_dir)
            ce_file = os.path.join(ce_dir, os.path.basename(

...
class BalancedLoss(nn.Module):
    ...
    def forward(self, input, target):
        pos_mask = (target == 1) # 正样本矩阵，曼哈顿距离 \leq 2 则为 1
        neg_mask = (target == 0) # 负样本矩阵，其它地方为 0
        pos_num = pos_mask.sum().float() # 正样本数目
        neg_num = neg_mask.sum().float() # 负样本数目
        weight = target.new_zeros(target.size())
        weight[pos_mask] = 1 / pos_num # 正样本处填充为 1 / pos_num
        weight[neg_mask] = 1 / neg_num * self.neg_weight # 负样本处填充为 1 / neg_num, self.neg_weight 是 1
        weight /= weight.sum() # 分布归一化。weight 用于平衡正负样本，使得正负样本的权重相等
